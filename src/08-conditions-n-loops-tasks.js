...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  if (a < b) {
    return `${startIncluded}${a}, ${b}${endIncluded}`;
  }
  return `${startIncluded}${b}, ${a}${endIncluded}`;
}
...
function isCreditCardNumber(ccn) {
  let res = 0;
  const temp = ccn.toString().split('').map(Number);
  for (let i = 0; i < temp.length; i += 1) {
    if ((temp.length - i) % 2 === 0) {
      temp[i] *= 2;
      if (temp[i] > 9) {
        temp[i] -= 9;
      }
...
function getDigitalRoot(num) {
  const res = num.toString().split('').reduce((a, b) => +a + +b);
  if (res < 10) {
    return +res;
  }
